...
eval_f <- function(x)
{
  return ( -( (1-capitalGainTax) *(x[1]*(1 + (bndYield*(1-incomeTax)) + bndAppr)^years - x[1]) + x[1]) -
             ...
             ((1-incomeTaxInRetirement)*x[4]*(1 + bndAppr + bndYield)^years) -
             ((1-incomeTaxInRetirement)*x[5]*(1 + stckAppr + stckYield)^years) -
             ((1-incomeTaxInRetirement)*x[6]*(1 + reitAppr + reitYield)^years)
  )
}
...
lb <- c(0,0,0,0,0,0)
ub <- c(4000,14000,2001,4000,14000,2000)
#initial values
x0 <- c(0,14000,0,4000,0,2000)
